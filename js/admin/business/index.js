...
//retrieve list
    ...
    //console.log(route);
      dataTable = $(table).DataTable({
       //"dom": 'T<"clear">lfrtip',
         ...
         buttons: [
            {
          text: '<span class="fa fa-plus fa-fw"></span>Alta de Empresa</a>',
  ...
  window.columnsDef = {
    buttons : function(data, type, full, meta){
      return (
       "<button id='show-business-"+full.id+"' type='button'"+
        "class='business-show btn btn-primary' data-value='"+full.document+
        "' data-id='"+full.id+"' ><i class='fa fa-eye'></i></button>"+
        ( (full.status==1) ?
        "<button id='delete-liq-"+full.period_liquidation_id+"' type='button'"+
         "class='business-delete btn btn-danger' data-code='"+full.code+
       ...
       1 :
     [{ data: 'id',"targets": 0, 'title':'id', visible:true },
    { data: 'field_name1',"targets": 1, 'title':'Nombre', visible:true},
    { data: 'document',"targets": 2, 'title':'Documento', visible:true},
    { data: 'zone_name',"targets": 3, 'title':'Provincia / Zona', visible:true},
    { data: 'activity_name',"targets": 4, 'title':'Actividad', visible:true},
    { data: 'month_close',"targets": 5, 'title':'Mes Cierre', visible:true},
    {
     'targets' : 6, 'title':'Ops', visible:true,
...
window.openModalModule = function(data_id, period, status, apply_to){
  ...
  /*if(status=='1'){
    add_on1 = '<button class="btn btn-danger" id="btn-prev-lock-business">'+
  ...
  if(status=='2'){
    add_on1 = '<button class="btn btn-primary" id="btn-unlock-business">'+
      '<span class="fa fa-unlock fa-fw"></span>Abrir</a></button>';
      $("#general-button-save").hide();
  }*/

  title1 = 'Formulario de Empresa';
...
window.openModalDelete = function(data_id, data_apply_to, data_code){
  ...
  .done(function(response, textStatus, jqXHR){ // What to do if we succeed
    ...
    if(jqXHR.status==200){
      ...
      $.ajax({
        url: "/js/admin/business/delete.js",
        dataType: "script"
      });
...
window.callback_module(1, '#principal-data');
  ...
  window.dataTable['#principal-data'].on( 'draw', function (e) {
    $(this).find('.business-show').off();
    //show element business
    $(this).find('.business-show').on('click', function(){
    ...
    /*
    $(this).find('.business-delete').off();
    $(this).find('.business-delete').on('click', function(){
